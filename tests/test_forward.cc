...
static bool TestForwardTokenProducesLogitShape() {
  MiniLlamaModel model = LoadModel("models/tiny/model.json", "models/tiny/model.bin");
...
static struct ForwardTestRegistrar {
  ForwardTestRegistrar() {
    RegisterTest("forward_token_produces_logit_shape",TestForwardTokenProducesLogitShape);
    RegisterTest("forward_token_changes_kv_cache", TestForwardTokenChangesKvCache);
    RegisterTest("generation_greedy_sequence", TestGenerationGreedySequence);
    RegisterTest("position_increments_correctly", TestPositionIncrementsCorrectly);
    RegisterTest("forward_rejects_invalid_token", TestForwardRejectsInvalidToken);
    RegisterTest("forward_rejects_invalid_position", TestForwardRejectsInvalidPosition);
  }
